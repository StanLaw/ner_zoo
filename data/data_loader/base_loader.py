from tqdm import tqdm
import random
import json

class BaseLoader:
	
	def __init__(self, filename, batch_size=64):
		self._data = []
		self.filename = filename
		self.batch_size = batch_size
	
	def load(self):
		with open(self.filename, 'r') as f:
			for line in tqdm(f.readlines(), desc="data loading"):
				_datum = self._parse_line(line)
				self._data.append(_datum)

	def _parse_line(self, line)
		return json.loads(line)

	@property
	def size(self):
		return len(self._data)

	def shuffle(self):
		random.shuffle(self._data)

	@staticmethod
	def _seperate(input_list, flag):
		if not flag:
			return input_list
		_names = []
		_label = []
		for one in input_list:
			_names.append(one.text)
			_label.append(one.label)
		return _names, _label

	def batch_iter(self, seperate=False):
		self.shuffle()
		k = len(self._data) // self.batch_size
		for i in range(k):
			yield self._seperate(self._data[i * self.batch_size: (i+1) * self.batch_size])
		if len(self._data) % self.batch_size > 0:
			yield self._seperate(self._data[k * self.batch_size: ])

	
